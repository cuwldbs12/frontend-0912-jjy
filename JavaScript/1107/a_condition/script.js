...
// if문은 주어진 조건문이 참(true)일 때만 코드 블럭을 실행
// 만약 조건이 거짓이면 코드 블럭은 무시
...
//! 2. if else문
// else문은 if문과 함께 사용
// , if의 조건이 거짓일 경우 else 뒤의 코드 블럭이 실행
...
// else if문의 예시
// 나이가 13살 미만일 경우 '미취학 아동입니다.' 출력
// 나이가 13살 이상 20살 미만일 경우 '청소년입니다.' 출력
// 나이가 20살 이상일 경우 '성인입니다.' 출력
...
// boolean 타입으로 false값으로 인식되는 값
// : false, 0, '', null, undefined
// 위의 값을 제외하고는 true값을 반환하기 때문에
// 변수가 참인지 또는 ㄱ밧이 존재하는지 테스트하기 위해서
// 변수 이름 그 자체를 사용할 수 있음.

let cheesse = 'Chedder';

if (cheesse) {
  console.log("Cheese avilable for making cheede on toast");
} else {
...
// switch (식) {
  //    case 값1:
  //    식이 값1 과 일치할 때 실행되는 코드
  //     break;
  //    case 값2:
  //    식이 값2 과 일치할 때 실행되는 코드
  //     break;
  //     ...
  //    default:
...
// and 연산자즌 조건식이 둘 이상일 경우
// , 하나만 false여도 최종 결과값이 false

// >> 첫번째 조건식이 false면 , 두번째 조건식은 체크하지 않고 false
// and 연산자 사용시 false가 될 확률이 높은 조건을 첫번째 조건식으로 사용하는 것을 권장

// or 연산자는 조건식이 둘 이상일 경우
// , 하나만 true여도 최종 결과값 true